...
from v3.model import Network

__author__ = 'wangj'
__date__ = '2018/05/03 16:53'
__doc__ = '''
增加了模型的保存和继续训练
增加了loss和accuracy的可视化
'''
# 定义模型存储的位置
CKPT_DIR = 'ckpt'
...
class Train(object):
    ...
    def train(self):
        ...
        # 可视化存储目录为当前文件夹下的 log
        mergerd_writer = tf.summary.FileWriter('./log',self.sess.graph)
        ...
        while step < train_step:
            ...
            _, loss, merged_summary = self.sess.run([self.net.train, self.net.loss, merged_summary_op], feed_dict={
                self.net.x: x,
            ...
            if step%100 == 0:
                mergerd_writer.add_summary(merged_summary,step)
