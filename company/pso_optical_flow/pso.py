...
if sys.argv[1] == 'test' or sys.argv[2] == 'test':
    ...
    sys.stderr.write(
        'USAGE: [-v] swarm.py num_particles inertia cognition social_rate local_rate'
        ' world_width world_height max_velocity max_epochs num_neighbors fname\n')
    sys.exit(0)
else:
    a_test = False
            ...
            if self.num_neighbors > 0:
                v_x = self.inertia * p.velocity_x + self.cognition * rand1 * (
                        p.personal_best[1] - p.x) + self.social_rate * rand2 * (
                              self.Particle.global_best[1] - p.x) + self.local_rate * rand3 * (p.local_best[1] - p.x)
                v_y = self.inertia * p.velocity_y + self.cognition * rand1 * (
                        p.personal_best[2] - p.y) + self.social_rate * rand2 * (
                              self.Particle.global_best[2] - p.y) + self.local_rate * rand3 * (p.local_best[2] - p.y)
